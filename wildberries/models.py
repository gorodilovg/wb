...
class ProductCardImage(models.Model):
    ...
    product_card = models.ForeignKey('wildberries.ProductCard', on_delete=models.CASCADE,
                                     related_name='images',
                                     null=True)
...
class ProductCard(IdentifierMixin):
    ...
    STATUS_CHOICES = [
        (PROCESSING, 'Информация о товаре добавляется в систему'),
        (MODERATING, 'Товар проходит модерацию'),
    ...
    ozon_product_id = models.PositiveIntegerField(default=0, blank=True)
    ozon_category_id = models.PositiveIntegerField(default=0, blank=True)

    wildberries_fbs_sku = models.IntegerField(blank=True, default=0)
    wildberries_product_id = models.CharField(default='', max_length=128, blank=True)
    wildberries_category_id = models.PositiveIntegerField(default=0, blank=True)
    wildberries_character_id = models.IntegerField(
        verbose_name="chrt_id", blank=True, default=0
    )
...
class Order(IdentifierMixin):
    OZON_FBO = 'ozon_fbo'
    OZON_FBS = 'ozon_fbs'
    WILDBERRIES_FBS = 'wildberries_fbs'
    ...
    STATUS_CHOICES = [
        (NO_INFORMATION, 'Информация о статусе отсутствует'),
